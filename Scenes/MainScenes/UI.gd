...
onready var wave_label = get_node("HUD/InfoBar/H/WaveNr")
...
func set_tower_preview(tower_type, mouse_position):
	...
	range_texture.position = Vector2(32,32)##gets the same offset as the towers
...
func update_health_bar(base_health):
	#hp_bar_tween.interpolate_property(node, parameter, start_value, end_value, duration, transition_type, easing_type) --> explaining the line below
...
func update_wave_counter(current_wave, numberOfWaves):
	wave_label.text = "Welle " + str(current_wave) + "/" + str(numberOfWaves)

##Optionen öffnen
func _on_Options_pressed():
	get_node("GameMenu/ButtonSound_Click").play()
